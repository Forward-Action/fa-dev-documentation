...
All documentation is found in the `files` directory. Edit the .md files here or add new ones using the (Honkit)[https://honkit.netlify.app/] documentation as a guide.
...
To see a local version of then site, run:
````
yarn serve
````
...
This will build the site structure using Honkit and output it to a directory named `_book`, delete the older `docs` directory, rename the new `_book` directory to `docs`.
